...
@section('content')
<form method="post" action="{{ route('user.deck.update',['id'=>$decks->id]) }}" enctype="multipart/form-data">

    <input type="hidden" name="_token" value="{{ csrf_token() }}" />

    <h2 class="title">Editar Deck</h2>

    <div class="form-group form-floating mb-3">
        <input type="text" class="form-control" name="deckname" placeholder="Nome do Deck" value="{{ $decks->deckname }}" required="required" autofocus>
...
!side 
15693423
" required="required" autofocus rows="8">{{ $decks->cards }}</textarea>
    ...
    <button class="w-100 btn btn-lg btn-dm-primary2" type="submit">Salvar Alteraçoes</button>
